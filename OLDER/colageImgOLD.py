...
def opacidadImagenes(PathImgOri ,PathImagenesRe, PathImagenAgua, con):
    ...
    # path, dirs, files = next(os.walk(r"./MosaicoPython/public/Img"))    
...
# CONDICIONALES DE #NUMERO DE IMAGENES QUE HAY EN LA CARPETA   
    if con == 1:
        ...
        resize(1, PathImgOri, PathImagenesRe, 1920, 1080)
    ...
    # CUANDO HAY 2 IMG
        ...
        for imagen in range(1,3):
            resize(imagen, pathImagenes, pathRecortes, 960, 1080)
        # COLLAGE
        im = 1
        for col in range(2):
            # print(reg,col)
            ...
            final[0 * 90: (1) * 1080, col * 960: (col+ 1) * 960] = concat_h1
            im += 1
            if im == 8:
                im = 1
    ...
    # CUANDO HAY 3 IMG
        ...
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 640, 1080)
        # COLLAGE
        im = 1
        for col in range(3):
            # print(reg,col)
            little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
            concat_h1 = cv2.hconcat([little])
            final[0 * 1080: (1) * 1080, col * 640: (col+ 1) * 640] = concat_h1
            im += 1
            if im == 8:
                im = 1
        concat_v = cv2.vconcat([concat_h1])
    ...
    # CUANDO HAY 4 IMAGENES
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 960, 540)
        im = 1
        for reg in range(2):
            for col in range(2):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 540: (reg + 1) * 540, col * 960: (col+ 1) * 960] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
        ...
        cv2.imshow('', total)
        cv2.waitKey(0)
    # CUANDO HAY 5 IMAGENES
    elif con == 5:
        # CAMBIAR EL TAMAÑO DE LA IMAGENES
        pathImagenes = PathImgOri
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 640, 540)
        im = 1
        for reg in range(2):
            for col in range(3):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 540: (reg + 1) * 540, col * 640: (col+ 1) * 640] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
    ...
    # CUANDO HAY 6 IMAGENES   
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 480, 540)
        im = 1
        for reg in range(2):
            for col in range(4):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 540: (reg + 1) * 540, col * 480: (col+ 1) * 480] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 640, 360)
        im = 1
        for reg in range(3):
            for col in range(3):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 360: (reg + 1) * 360, col * 640: (col+ 1) * 640] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 384, 540)
        im = 1
        for reg in range(2):
            for col in range(5):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 540: (reg + 1) * 540, col * 384: (col+ 1) * 384] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 480, 360)
        im = 1
        for reg in range(3):
            for col in range(4):
                # print(reg,col)
                little = cv2.imread(PathImagenesRe+r"/"+str(im)+".jpg")
                concat_h1 = cv2.hconcat([little])
                final[reg * 360: (reg + 1) * 360, col * 480: (col+ 1) * 480] = concat_h1
                im += 1
                if im == (con+1):
                    im = 1
            concat_v = cv2.vconcat([concat_h1])
        ...
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 480, 270)

        concantenar(PathImagenesRe, 4, 4, 480, 270, final, con = con)
        ...
        cv2.imshow('', total)
        cv2.waitKey(0)      
    elif(con < 26 and con > 16):
        # CAMBIAR EL TAMAÑO DE LA IMAGENES
        print("Hola")
        pathImagenes = PathImgOri
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 384, 216)
        concantenar(PathImagenesRe, 5, 5, 384, 216, final, con = con)
        img1 = cv2.imread("tlaxcalaFin.jpg")
        total = cv2.addWeighted(src1=img1, alpha=1, src2=img2, beta=0.5, gamma=0)
        cv2.imwrite(r"fin"+str(con)+r".jpg",total)
        cv2.imshow('', total)
        cv2.waitKey(0) 
    elif(con < 37 and con > 25):
        # CAMBIAR EL TAMAÑO DE LA IMAGENES
        print("6")
        pathImagenes = PathImgOri
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 320, 180)
        concantenar(PathImagenesRe, 6, 6, 320, 180, final, con = con)
        img1 = cv2.imread("tlaxcalaFin.jpg")
        total = cv2.addWeighted(src1=img1, alpha=1, src2=img2, beta=0.5, gamma=0)
        cv2.imwrite(r"fin"+str(con)+r".jpg",total)
        cv2.imshow('', total)
        cv2.waitKey(0)
    elif(con < 65 and con > 36):
        # CAMBIAR EL TAMAÑO DE LA IMAGENES
        print("8")
        pathImagenes = PathImgOri
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 240, 135)
        concantenar(PathImagenesRe, 8, 8, 240, 135, final, con = con)
        img1 = cv2.imread("tlaxcalaFin.jpg")
        total = cv2.addWeighted(src1=img1, alpha=1, src2=img2, beta=0.5, gamma=0)
        cv2.imwrite(r"fin"+str(con)+r".jpg",total)
        cv2.imshow('', total)
        cv2.waitKey(0)
    elif(con < 101 and con > 65):
        # CAMBIAR EL TAMAÑO DE LA IMAGENES
        print("10")
        pathImagenes = PathImgOri
        pathRecortes = PathImagenesRe
        for imagen in range(1, con + 1):
            resize(imagen, pathImagenes, pathRecortes, 192, 108)
        concantenar(PathImagenesRe, 10, 10, 192, 108, final, con = con)
        img1 = cv2.imread("tlaxcalaFin.jpg")
        # total = cv2.addWeighted(src1=img1, alpha=1, src2=img2, beta=0.5, gamma=0)
